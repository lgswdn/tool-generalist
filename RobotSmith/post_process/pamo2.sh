#!/bin/bash
# ──────────────────────────────────────────────────────────────
# pamodet.sh  –  Run PAMO decimation on deterministic tool OBJ files
#
# Usage:
#   bash pamodet.sh [EEF_DIR] [TRIAL_DIR_NAME] [PAMO_DIR]
#   bash pamodet.sh ../eef tmp_trial ~/project/pamo
#   bash pamodet.sh                    # defaults: ../eef tmp_trial $HOME/project/pamo
#
# Environment variables (optional):
#   MAX_JOBS      – max parallel workers (default: number of available GPUs)
#   MAX_RETRIES   – retries per job on failure (default: 5)
...
TRIAL_DIR_NAME="${2:-tmp_trial}"
PAMO_DIR="${3:-${PAMO_DIR:-$HOME/project/pamo}}"
TRIAL_ROOT="${EEF_DIR}/${TRIAL_DIR_NAME}"
...
if [ ! -d "$TRIAL_ROOT" ]; then
    echo "ERROR: $TRIAL_ROOT does not exist. Nothing to process."
    exit 1
fi
...
echo "TRIAL_DIR_NAME = $TRIAL_DIR_NAME"
...
for trial_dir in "$TRIAL_ROOT"/*; do
    # Skip if it's not a directory
    [ -d "$trial_dir" ] || continue

    # Use the trial subdirectory name as the output prefix (e.g., 000).
    dir_name=$(basename "$trial_dir")
    x="$dir_name"

    # Loop through all deterministic and randomized variant OBJ files in this directory.
    for obj_file in "$trial_dir"/*_det_*.obj "$trial_dir"/*_var_*.obj; do
        # Skip if no matching files are found
        [ -f "$obj_file" ] || continue

        # Extract the filename without the path (e.g., apple_det_002.obj or apple_var_002.obj)
        filename=$(basename "$obj_file")
        
        # Remove the .obj extension (e.g., apple_det_002 or apple_var_002)
        base_name="${filename%.obj}"

        if [[ "$base_name" == *_det_* ]]; then
            kind="det"
            i_padded="${base_name##*_det_}"
            name="${base_name%_det_*}"
        elif [[ "$base_name" == *_var_* ]]; then
            kind="var"
            i_padded="${base_name##*_var_}"
            name="${base_name%_var_*}"
        else
            echo "Warning: Skipping unexpected OBJ filename: $obj_file"
            continue
        fi

        # Construct the output paths
        OUTPUT_OBJ="${OUT_OBJ_DIR}/${x}_${name}_${kind}_${i_padded}.obj"
        INPUT_JSON="${trial_dir}/${name}_${kind}_${i_padded}_metadata.json"
        OUTPUT_JSON="${OUT_META_DIR}/${x}_${name}_${kind}_${i_padded}_metadata.json"
