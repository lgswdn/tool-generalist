...
for trial_dir in "$EEF_DIR"/tmp_trial/*; do
    ...
    for obj_file in "$trial_dir"/*_det_*.obj; do
        ...
        OUTPUT_OBJ="${OUT_OBJ_DIR}/${x}_${name}_det_${i_padded}.obj"
        INPUT_JSON="${trial_dir}/${name}_det_${i_padded}_metadata.json"
        OUTPUT_JSON="${OUT_META_DIR}/${x}_${name}_det_${i_padded}_metadata.json"
