...
def adjust_gripper_pose_without_plan(scene, robotarm, pos, quat, save_img, q_gripper, vis=False, iter=None, img_steps=0, img_save_dir='.'):
    '''
    Args:
    ...
    for _ in range(200):
        ...
        #     save_img()
        if vis and _ % 25 == 0:
        #     print('adj_pos', robotarm.get_link(ee_name).get_pos())
        #     print('adj_quat', robotarm.get_link(ee_name).get_quat())
            img_steps=save_img(img_steps=img_steps, iter=iter, img_save_dir=img_save_dir)
    return img_steps
...
def my_grasp(scene, robotarm, obj, save_img, restriction_x = None, restriction_y = None, restriction_z = None, grasp_quat=None, filename=None, verbose=False):
    ...
    if isinstance(obj, MPMEntity):
        ...
        # mold_qpos = np.load("gw_mold_605c389c-18d2-11f0-8e09-bd91f6ecbcf0.npy")
        # for waypoint in mold_qpos:
        #     robotarm.control_dofs_position(waypoint)
        #     for _ in range(30):
        #         scene.step()
        # return
    ...
    for point_pair in sampled_points:
        ...
        for normal in normal_list:
            ...
            for ik_try_idx in range(try_ik_times):
                ...
                if verbose:
                    img_steps=save_img(f'test_{ik_try_idx}_norm{normal[0].item()}{normal[1].item()}{normal[2].item()}_ikpos{ik_pos[0].item():.2f}{ik_pos[1].item():.2f}{ik_pos[2].item():.2f}_{collision}_{torch.norm(err).item():.2f}')
