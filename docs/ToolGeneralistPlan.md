...
重构后的代码库不重构工具生成管线（并且工具生成管线现在不在这个电脑上，你不需要关心），只消费类似 `/mnt/project/world_model/tool_generalist/eef` 文件夹中的、生成出的稳定产物。具体的工具文件夹路径从 `ExpCfg.paths_yaml` 指向的 `configs/paths/*.yaml` 读取。工具相关代码必须把下面几项当作明确接口，而不是从目录结构或旧脚本里猜：
...
2. Gripper 数据只从 `tools_adjusted.json` 指向的 `contact_tip.obj` 采样 tool anchors；缺失 terminal contact mesh 时立即失败，不退回整工具表面。
...
- `configs/paths/*.yaml` 只存机器相关绝对路径；实验参数不放在路径配置中。
- 物体资产目录、`objects_selected.json`、工具资产目录、tool USD/robot USD 根目录、`tools_adjusted.json`、`tools_selected.json` 和 `meshdata_adjusted_root` 的绝对路径属于 `configs/paths/*.yaml`。
...
1. 加载 `ExpCfg`。
2. 加载 `ExpCfg.paths_yaml`（默认 `configs/paths/default.yaml`）。
3. 展开并 freeze 全部子配置。
4. 校验：
   - GPU 数量。
   - 路径存在。
...
| `object_mesh_path` | `str` | yes | - | object mesh 路径。路径应可由 `ExpCfg.paths_yaml` 指向配置中的数据根目录解析。 |
...
| 路径存在 | `object_mesh_path`、`tool_mesh_path` 必须能通过路径配置或绝对路径解析到文件。 |
...
#### `PathCfg` / `configs/paths/*.yaml`

路径配置只放机器相关绝对路径，建议至少包含：
...
| `timestamp` | run 创建时间。 |
| `paths_yaml` | 使用的 `configs/paths/*.yaml` 配置路径。 |
...
| `require_tool_tip_anchor` | 强制从显式 terminal contact mesh 采样；缺失时立即失败。 |
