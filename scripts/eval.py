"""Config-only RL evaluation wrapper."""
...
def run_rl_evaluation(
    ...
    spec = build_rl_runtime_spec(exp_cfg, paths, artifact_dir, mode="eval")
    artifact_path = Path(artifact_dir)
    artifact_path.mkdir(parents=True, exist_ok=True)
    write_json(artifact_path / "rl_eval_runtime_spec.json", asdict(spec))
    return asdict(spec)
