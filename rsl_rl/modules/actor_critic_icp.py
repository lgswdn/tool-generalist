...
class ActorCriticICP(nn.Module):
        ...
        **kwargs,
        ...
        if icp_weights_path is not None:
            ...
            default_cfg = ICPNet.Config(
                ...
                keys={
                    "hand_state": 9
                },  # From tokenize weights: hand_state.linear.weight: [128, 9]
            ...
            default_cfg.encoder.num_hidden_layers = (
                2  # From weights: Layer numbers: [0, 1]
            )
            default_cfg.encoder.layer.hidden_size = 128  # Match encoder_channel
            default_cfg.encoder.layer.num_attention_heads = (
                3  # From weights: 384/128 = 3 heads
            )
            ...
            print(
                "Loading ICP weights with strict=True to ensure configuration match..."
            )
            ...
            print(
                f"  - num_hidden_layers: {self.icp_encoder.cfg.encoder.num_hidden_layers}"
            )
            print(f"  - hidden_size: {self.icp_encoder.cfg.encoder.layer.hidden_size}")
            print(
                f"  - num_attention_heads: {self.icp_encoder.cfg.encoder.layer.num_attention_heads}"
            )
        ...
        print(
            f"ICP encoder cfg.encoder.layer.hidden_size: {self.icp_encoder.cfg.encoder.layer.hidden_size}"
        )
        ...
        if self.use_sd_cross:
            ...
            print(
                f"Using StateDependentCrossFeatNet for fusion. SD output dim: {sd_out_dim}, Fusion output dim: {fusion_hidden_dims[-1]}"
            )
        else:
        ...
        print(
            f"Fusion normalization: {fusion_norm_type if fusion_use_norm else 'None'}"
        )
        print(f"Actor normalization: {actor_norm_type if actor_use_norm else 'None'}")
        print(
            f"Actor output activation: {'Tanh' if actor_output_activation else 'None'}"
        )
        print(
            f"Critic normalization: {critic_norm_type if critic_use_norm else 'None'}"
        )
        ...
        for i in range(len(hidden_dims)):
            if i == len(hidden_dims) - 1:
                ...
                if is_actor and output_activation:
                    layers.append(
                        nn.Tanh()
                    )  # Actor uses tanh for bounded actions (optional)
    ...
    def _extract_point_cloud_and_context(self, observations):
        ...
        if isinstance(observations, torch.Tensor):
            ...
            raise ValueError(
                "observations must be a tensor for flattened observation mode"
            )
    ...
    def _get_fused_features(self, observations):
        ...
        point_cloud, context, regular_obs = self._extract_point_cloud_and_context(
            observations
        )
        ...
        if self.use_sd_cross:
            ...
            if len(icp_features.shape) > 2:
                icp_features = icp_features.reshape(icp_features.shape[0], -1)
            ...
            if regular_obs.shape[-1] > 0:
                ...
                batch_size = (
                    icp_features.shape[0]
                    if icp_features.shape[-1] > 0
                    else point_cloud.shape[0]
                )
                device = (
                    icp_features.device
                    if icp_features.shape[-1] > 0
                    else point_cloud.device
                )
                if icp_features.shape[-1] > 0:
                    zeros = torch.zeros(
                        batch_size,
    ...
    def train(self, mode=True):
        ...
        if hasattr(self, "icp_encoder"):
            frozen = not any(
                param.requires_grad for param in self.icp_encoder.parameters()
            )
            if frozen:
