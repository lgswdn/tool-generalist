...
from typing import Tuple, Optional, Iterable, List, Union, Mapping, Dict, Any, Callable
from dataclasses import dataclass, is_dataclass, fields, replace, field
...
try:
    ...
    print("[WARN] flash-attn not available in common.py")
...
class SingleGRU(nn.Module):
    ...
    def init_memory(self, batch_shape: Tuple[int, ...] = (), **kwds):
...
class DeepGRU(nn.Module):
    ...
    def init_memory(self, batch_shape: Tuple[int, ...] = (), **kwds):
        ...
        self.register_buffer(
            "memory", th.zeros((self.num_layer, *batch_shape, self.state_size), **kwds)
        )
        return self.memory
...
class CountDormantNeurons:
    ...
    def _on_fwd(self, model, input: th.Tensor, output: th.Tensor, name: str = None):
        h = output.detach()
        ...
        self._stats[name].append((s_numer / s_denom))
...
class FlipFlopReLU6(nn.Module):
    ...
    def __init__(self, inplace: bool = False):
        ...
        self.neg = nn.modules.activation.Hardtanh(-6.0, 0.0, inplace=inplace)
        self.pos = nn.modules.activation.Hardtanh(0.0, 6.0, inplace=inplace)
...
def _map_struct(
    ...
    """
    map_element = partial(
        map_struct, base_cls=base_cls, base_fn=base_fn, dict_cls=dict_cls
    )
        ...
        if is_dataclass(dst):
            ...
            for field in fields(src):
                setattr(
                    dst,
                    field.name,
                    map_element(getattr(src, field.name), op, getattr(dst, field.name)),
                )
            return dst
        else:
            ...
            out_dict = {}
            for field in fields(src):
                out_dict[field.name] = map_element(getattr(src, field.name), op, None)
    ...
    if isinstance(src, Iterable):
        if isinstance(dst, Iterable) and len(src) == len(dst):
            out = [map_element(vs, op, vd) for (vs, vd) in zip(src, dst)]
        else:
...
map_tensor = partial(map_struct, base_cls=th.Tensor)
...
def dclamp(
    input: th.Tensor, min: Union[float, th.Tensor], max: Union[float, th.Tensor]
):
            ...
            if (max_grad_norm is not None) and (parameters is not None):
                ...
                try:
                    grad_norm = nn.utils.clip_grad_norm_(
                        parameters, max_grad_norm, error_if_nonfinite=skip_nan
                    )
                except RuntimeError:
                    skip_step = True
                if not skip_step:
...
def hard_update(src: nn.Module, dst: nn.Module):
...
class MultiHeadLinear(nn.Module):
    ...
    def __init__(
        ...
        **kwds,
        ...
        super().__init__()
        self.linear = nn.Linear(dim_in, num_head * dim_head, *args, **kwds)
        self.h = num_head
        self.d = dim_head
        self.unbind = unbind
...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        for d0, d1 in zip(dims[:-2], dims[1:-1]):
            ...
            layers.extend(
                (
                    LinearBn(d0, d1, use_bn=use_bn, bias=layer_bias, use_ln=use_ln),
                    act_cls(),
                )
            )
        if activate_output:
            layers.extend(
                (
                    LinearBn(
                        dims[-2], dims[-1], use_bn=use_bn, bias=bias, use_ln=use_ln
                    ),
                    act_cls(),
                )
            )
            ...
            # hardcoding... for now
            layers.extend((nn.Linear(dims[-2], dims[-1], bias=bias),))
...
class ConvBlock(nn.Module):
    ...
    class Config(ConfigBase):
        ...
        def dim_out(self, dim_in: Tuple[int, int, int]) -> Tuple[int, int, int]:
            return conv2d_dim_out(
                dim_in, self.channels, self.kernel_size, self.stride, self.padding, 1
            )
    ...
    def __init__(self, cfg: Config, in_channels: int):
        ...
        self.conv = nn.Sequential(
            coord,
            nn.Conv2d(
                c_in, cfg.channels, cfg.kernel_size, cfg.stride, cfg.padding, bias=bias
            ),
            bn,
            act_cls(),
        )
...
class SimpleCNN(nn.Module):
    ...
    def __init__(self, blocks: Tuple[ConvBlock.Config, ...], in_channels: int):
        super().__init__()
...
class CNN(nn.Module):
    def __init__(
        ...
        for hid_out in blocks:
            layers.extend(
                [
                    nn.Conv2d(hid_in, hid_out, 3, padding=1, bias=False),
            ...
            hid_in = hid_out
        self.net = nn.Sequential(*layers)
        self.activate = nn.ReLU(True) if activate else nn.Identity()
...
class CBA2D(nn.Module):
    ...
    def __init__(self, **kwds):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(**kwds), nn.BatchNorm2d(kwds["out_channels"]), nn.LeakyReLU()
        )
...
class CBAR2D(nn.Module):
    ...
    def __init__(self, **kwds):
        super().__init__()
        self.model = nn.Sequential(CBA2D(**kwds), ResidualBlock(kwds["out_channels"]))
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.to_q = EinMix(
            "b q d -> b h q k", weight_shape="d h k", d=dim_x, k=dim_k, h=num_h
        )
        self.to_k = EinMix(
            "b d -> b h n k", weight_shape="d h n k", d=dim_c, k=dim_k, h=num_h, n=num_k
        )
        self.to_v = EinMix(
            "b d -> b h n v", weight_shape="d h n v", d=dim_c, v=dim_v, h=num_h, n=num_k
        )
        ...
        self.mix = nn.Sequential(
            ...
            # 1x1 convolution.
            EinMix("b h q v -> b q h v0", weight_shape="v v0", v=dim_v, v0=dim_v0),
            # Mix between heads.
            # Sort of like pointwise convolution.
            EinMix("b q h v -> b q (h0 v)", weight_shape="h h0", h=num_h, h0=dim_h0),
        )

    def forward(self, context: th.Tensor, inputs: th.Tensor) -> th.Tensor:
        """
        Args:
            context: (b, d_c)
            inputs:   (b, q, d_x)
...
class CrossAttention(nn.Module):
    def __init__(
        self, embed_size, num_heads=4, softmax_scale=None, attention_dropout=0.0
    ):
        super().__init__()
    ...
    def forward(self, query, key, value, key_padding_mask=None, causal=False):
        ...
        # Reshape for multi-head attention: (B, S, E) -> (B, S, H, D)
        ...
        # Use flash-attn 2.x if available, otherwise fallback to standard attention
        if FLASH_ATTN_AVAILABLE:
            ...
            attn_probs = th.dropout(
                attn_probs,
                p=self.attention_dropout if self.training else 0.0,
                training=self.training,
            )

            output = th.matmul(attn_probs, V)  # (B, H, S_q, D)
            output = output.transpose(1, 2)  # (B, S_q, H, D)

        # 重塑输出并进行输出投影: (B, S_q, H, D) -> (B, S_q, E)
        output = output.contiguous().view(batch_size, seq_len_q, self.embed_size)
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        self.cross_attention = CrossAttention(
            cfg.embed_size, cfg.num_heads, cfg.softmax_scale, cfg.attention_dropout
        )
    ...
    def forward(self, query, key, value, key_padding_mask=None, causal=False):
        ...
        attention_output = self.cross_attention(
            query, key, value, key_padding_mask=key_padding_mask, causal=causal
        )
        attention_output = self.layernorm1(
...
class CrossAttensionDecoder(nn.Module):
    ...
    class Config(ConfigBase):
        ...
        layer: CrossAttentionLayer.Config = field(
            default_factory=CrossAttentionLayer.Config
        )
    ...
    def __init__(self, cfg: Config):
        ...
        self.layers = nn.ModuleList(
            [CrossAttentionLayer(cfg.layer) for _ in range(cfg.num_hidden_layers)]
        )
...
class PosEncodingSine(nn.Module):
    ...
    def __init__(self, dim_in: int, dim_out: int, scale: float = 30.0):
        super().__init__()
...
class PosEncodingMLP(nn.Module):
    def __init__(
        ...
        # self.linear = nn.Linear(dim_in, dim_out)
        self.mlp = MLP(
            merge_shapes(dim_in, dim_hidden, dim_out),
            act_cls=get_activation_function(act_cls),
...
class PosEncodingNeRF(nn.Module):
    ...
    def __init__(
        ...
        # (2*F+1)*D
        self.out_dim = dim_in * ((1 if cat_input else 0) + 2 * self.num_frequencies)

        # Precompute the coefficient multipliers.
        self.register_buffer(
            "coefs",
            th.as_tensor(
                np.pi * (2 ** th.arange(self.num_frequencies)), dtype=th.float
            ),
        )
...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    def __init__(
        self, dim_in: int, dim_out: int, flatten: bool = True, pad: bool = False
    ):
        """
        Args:
            dim_in: dimensionality of coordinate input.
...
def ortho_init(m: nn.Module, gain: float, target_cls: Tuple[nn.Module] = None):
    """
...
def transfer(
    ...
    renamed_state_dict = {
        _replace_prefix(k, prefix_map): v for (k, v) in state_dict.items()
    }
...
def replace_layer(model: nn.Module, old_layer_cls, new_layer_fn):
    ...
    for parent_layer, last_token in get_all_parent_layers(model, old_layer_cls):
...
def test_mha():
    ...
    mha = MultiHeadCrossAttention(
        dim_c, dim_x, dim_k, dim_v, dim_v, num_k=num_k, num_h=num_h
    )
...
def test_map_struct_with_dataclass():
    ...
    dst = map_struct(src, lambda src, dst: 4, dst=None)
    print(src)
    print(dst)

    dst = A(a=2, b=6)
    dst2 = map_struct(src, lambda src, dst: src + 5, dst=dst)
    print(src)
    print(dst)
    print(dst2)
...
def test_attention():
    ...
    # print_memory(1)
    k = th.randn((num_b, num_k, 1, dim_k), dtype=th.float32, device=device)
    v = th.randn((num_b, num_k, 1, dim_v), dtype=th.float32, device=device)
    q = th.randn((num_b, num_q, 1, dim_k), dtype=th.float32, device=device)

    print("attention")
    print_memory(1)
    o1 = attention(q.squeeze(dim=-2), k.squeeze(dim=-2), v.squeeze(dim=-2))
    print_memory(1)
