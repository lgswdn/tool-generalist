...
from rsl_rl.modules.models.common import (transfer, MultiHeadLinear, MLP)
from rsl_rl.modules.train.ckpt import load_ckpt, last_ckpt
from rsl_rl.modules.util.config import recursive_replace_map, ConfigBase
...
class ICPNet(nn.Module):
    ...
    class Config(FeatureBase.Config):
        ...
        def __init__(self, **kwds):
            ...
            print(f'ICP config: {names}')
            for k, v in kwds.items():
                if k in names:
                    setattr(self, k, v)
                    print(f"Setting {k} to {v}")
            # Ensure encoder is initialized if not provided
            if 'encoder' not in kwds:
                self.encoder = PointMAEEncoder.Config()
            self.__post_init__()
    ...
    def load(self, filename=None, params=None, verbose=False):
        ...
        self.headers.load_state_dict(new_header, strict=True)  # Force strict checking
        if verbose:
            print(f"loading parameter for : {pretrained_header.keys()}")
            print(f"Headers loaded with strict=True")
