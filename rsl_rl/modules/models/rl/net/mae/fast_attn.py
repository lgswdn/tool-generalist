...
def transfer_weights(src: ViTMAESelfAttention, dst: "ViTMAESelfFlashAttention"):
    # weight = shape (out, in)
    with th.no_grad():
...
def replace_self_attention(model: nn.Module, config=None):
    ...
    for parent_layer, last_token in get_all_parent_layers(model, ViTMAESelfAttention):
        source = getattr(parent_layer, last_token)
        weight = source.query.weight
        attn = ViTMAESelfFlashAttention(
            config, device=weight.device, dtype=weight.dtype
        )
...
class ViTMAESelfFlashAttention(nn.Module):
    def __init__(self, config: ViTMAEConfig, device=None, dtype=None) -> None:
        super().__init__()
    ...
    def forward(
        ...
        # slow path (fallback or when attention weights needed)
        if output_attentions or not FLASH_ATTN_AVAILABLE:
            return self.forward_slow(hidden_states, head_mask, output_attentions)

        # fast path with flash-attn 2.x
        qkv = rearrange(
            self.qkv(hidden_states),
        ...
        # flash_attn_qkvpacked_func expects: (batch, seqlen, 3, nheads, headdim)
...
def compare():
    ...
    B: int = 1
    S: int = 4
    x = th.randn((B, S, cfg.hidden_size), dtype=th.float, device=device)
